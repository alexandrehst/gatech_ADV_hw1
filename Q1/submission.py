...
class  TMDBAPIUtils:
    ...
    def get_response(self, conn_name, id):

        req = http.client.HTTPSConnection("api.themoviedb.org")
        payload = ''
        headers = {}
        conn_string = ''
    ...
    def get_movie_cast(self, movie_id:str, limit:int=None, exclude_ids:list=None) -> list:
        ...
        res = self.get_response( 'cast', movie_id)
        ...
        if exclude_ids is not None:
            cast = list(filter(lambda c: c['id'] not in exclude_ids, cast))
    ...
    def get_movie_credits_for_person(self, person_id:str, vote_avg_threshold:float=None)->list:
        ...
        res = self.get_response( 'credits', person_id)
        if res.status != 200:
            return None
        ...
        new_cast = list(filter(lambda c: c['vote_average'] >= vote_avg_threshold, cast))
